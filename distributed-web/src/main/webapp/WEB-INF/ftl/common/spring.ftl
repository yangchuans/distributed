...
<#macro formRadioButtonsSpan path options separator attributes="">
<@bind path/>
    <#list options?keys as value>
        <#assign id="${status.expression}${value_index}">
            <input style="width:5%;display:inline-block;margin-left:" type="radio" id="${id}" name="${status.expression}"
                   value="${value?html}"<#if stringStatusValue == value>
                   checked="checked"</#if> ${attributes}<@closeTag/>
    <span style="font-size:14px">${options[value]?html}</span>${separator}
    </#list>
</#macro>
